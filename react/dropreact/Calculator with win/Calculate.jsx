...
export default class Calculate extends React.Component {
    ...
    ResultValue() {
        this.state.count++;
        if (this.state.count % 10 === 0) { this.state.indicatorCalc = false; document.getElementById('aud').play(); }
        else { this.state.indicatorCalc = true; document.getElementById('aud').pause(); }
        switch (this.state.select) {
            case '+':
                return this.props.onEqualsClick({ count: this.state.count, input1: this.state.input1, selelct: this.state.select, input2: this.state.input2, result: this.state.result = Number(this.state.input1) + Number(this.state.input2) }, this.state.indicatorCalc)
            case '-':
                return this.props.onEqualsClick({ count: this.state.count, input1: this.state.input1, selelct: this.state.select, input2: this.state.input2, result: this.state.result = Number(this.state.input1) - Number(this.state.input2) }, this.state.indicatorCalc)
            case '*':
                return this.props.onEqualsClick({ count: this.state.count, input1: this.state.input1, selelct: this.state.select, input2: this.state.input2, result: this.state.result = Number(this.state.input1) * Number(this.state.input2) }, this.state.indicatorCalc)
